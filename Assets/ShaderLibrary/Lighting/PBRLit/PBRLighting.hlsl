...
{
	float ndotd = saturate(dot(surface.Normal, dir));
...
float GeometrySmith(PBRSurface surface, PBRLight light, float3 viewDir)
{
	...
	float G = ShlickGGX(surface, viewDir, k) * ShlickGGX(surface, light.LightDirection, k);
	return G;
}
...
float3 CalcualteDirectionLightSpecColor(PBRSurface surface, PBRLight light, float3 halfVector, float3 viewDir)
{
	...
	float NdotV = DotMax(surface.Normal, viewDir, 0.000001);
	float NdotL = DotMax(surface.Normal, light.LightDirection, 0.000001);
...
float3 CalcualteDirectionLight(PBRSurface surface, PBRLight light, float3 halfVector, float3 viewDir)
{
	...
	float nl = max(saturate(dot(surface.Normal, light.LightDirection)), 0.000001);
...
float3 CalculateInDirectionSpecColor(PBRSurface surface, PBRLight light, float3 halfVector, float3 viewDir)
{
	...
	float3 reflectVec = reflect(-viewDir, surface.Normal);
	...
	float nv = max(saturate(dot(surface.Normal, viewDir)), 0.000001);
	float t = pow((1 - nv), 5);
